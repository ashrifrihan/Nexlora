...
function StickyShellClient({ index, total, isMobile, children }: {
  ...
  return (
    <div ref={ref} className="relative h-full" style={isMobile ? { position: "sticky", top: 80 + index * 6, zIndex: index + 1, willChange: "transform" } : undefined}>
      <motion.div className="h-full bg-[#0c0c0e] rounded-[24px] shadow-[0_12px_40px_rgba(0,0,0,0.9)]" style={isMobile ? { scale, transformOrigin: "top center", willChange: "transform" } : undefined}>
        {children}
      </motion.div>
    </div>
  );
